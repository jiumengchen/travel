  ...
  <a-layout class="container">
      ...
      <div>
        <a-menu v-model:selectedKeys="selectedKeys" mode="inline">
          <template v-for="item in routeArr" :key="item.path">
            <a-menu-item @click="changeRouter(item.path)" v-if="item.meta?.hidden != true && !item.children"
              :key="item.path">
              <template #icon>
                <div class="iconfont" style="display: inline-block;" :class="[item.meta?.icon]"></div>
              </template>
            ...
            <a-sub-menu v-if="item.meta?.hidden != true && item.children" :key="item.path">
              ...
              <template v-for="children in item.children" :key="children.path">
                <a-menu-item @click="changeRouter(children.path)">
                  <template #icon>
                    <div class="iconfont" style="display: inline-block;" :class="[children.meta?.icon]">
                    </div>
                  </template>
    ...
    <a-layout>
      <a-layout-header class="header">
        ...
        <div>
          <a-avatar src="https://q1.itc.cn/q_70/images03/20250701/afddfb3d5fcf459594cfa880445c9b2c.jpeg" />
          <a-dropdown>
